...
class Editer(object):
    ...
    def __init__(
        self, root_path, head="https://www.bilinovel.com", book_no="0000", volume_no=1
    ):

        self.header = {'User-Agent': 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Mobile Safari/537.36', 'referer': head}
        ...
        # 通过 JavaScript 修改 navigator.platform
        # 如果不设置会导致网页一直转跳，也可以现在将首选语言设为英文
        ...
        try:
            self.cover_url = re.search(
                r"src=\"(.*?)\"", str(bf.find("div", {"class": "module-item-cover"}))
            ).group(1)
        except:
            self.cover_url = 'cid'
    ...
    def get_epub(self):
        ...
        epub_file = self.epub_path + '/' + check_chars(self.title + '-' + f'[{self.volume_no}]' + self.volume['book_name']) + '.epub'
