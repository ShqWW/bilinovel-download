...
class Editer(object):
    def __init__(self, root_path, book_no='0000', volume_no=1, interval=0, num_thread=1):
        ...
        secret_url = 'https://www.bilinovel.com/themes/zhmb/js/readtools.js'
        secret_html = self.get_html(secret_url, use_mobile=True)
        self.secret_map = get_secret_map(secret_html)
    ...
    def get_page_text(self, content_html):
        ...
        text_with_head = bf.find('div', {'id': 'acontent', 'class': 'bcontent'})
        ...
        if is_tansfer_rubbish_code:
            text_html = replace_rubbish_text(text_html)
    ...
    def get_content(self):
        ...
        content_htmls = get_content_html(self.title + '-' + self.volume['book_name'], self.author, self.publisher, self.tag_list, num_chap, num_img, self.is_color_page)
    ...
    def get_epub_head(self):
        ...
        mimetypefile = os.path.join(self.temp_path + '/mimetype')
    ...
    def get_epub(self):
        epub_file = os.path.join(self.epub_path, check_chars(self.title) + '-' + check_chars(self.volume['book_name']) + '.epub')
