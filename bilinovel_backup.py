...
class Editer(object):
    ...
    def get_page_text(self, content_html):
        ...
        text_with_head.find('div', {'class': 'cgo'}).extract()
        text_html = str(text_with_head)
        img_urlre_list = re.findall(r"<img class=\"imagecontent.*?>", text_html)
        for img_urlre in img_urlre_list:
            img_url = re.search(r'src="(.*?)"', img_urlre).group(1).replace('img1', 'img3')
            img_url_name = re.search(r'.com/(.*?).jpg', text_html).group(1)
            ...
            img_symbol = f'<p>[img:{self.img_url_map[img_url_name][1]}]</p>'
            if '00' in img_symbol:
        ...
        # text_html = restore_chars(text_html)
        # print(text_html)
        text = BeautifulSoup(text_html, 'html.parser').get_text()[1:-1]
        # print(text)
        
        # print(text.encode('unicode_escape').decode())
        text = restore_chars(text)
        # print(text)
        # print(self.img_url_map)
        # exit(0)
