...
from backend.bilimanga.bilimanga_router import *
...
class MainThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            color_page = self.parent.editline_color_page.text()
            out_path = read_config_dict('download_path')
            interval = read_config_dict('interval')
            downloader_router(out_path, book_no, volumn_no, interval, color_page, True, self.parent.hang_signal, self.parent.progressring_signal, self.parent.cover_signal, self.parent.editline_hang)
...
class MangaWidget(QFrame):
    ...
    def __init__(self, text: str, parent=None):
        ...
        self.parent = parent
        self.label_book = SubtitleLabel('漫号：', self)
        self.label_volumn = SubtitleLabel('卷号：', self)
        self.label_color_page = SubtitleLabel('彩页数量：', self)
        self.editline_book = LineEdit(self) 
        self.editline_volumn = LineEdit(self) 
        self.editline_color_page = LineEdit(self) 
        ...
        self.editline_color_page.setValidator(validator)
        ...
        self.label_color_page.setFont(font_label)
        self.editline_book.setFont(font_label)
        self.editline_volumn.setFont(font_label)
        self.editline_color_page.setFont(font_label)
        ...
        self.gridLayout.addWidget(self.editline_color_page, 2, 1)

        self.gridLayout.addWidget(self.label_book, 0, 0)
        self.gridLayout.addWidget(self.label_volumn, 1, 0)
        self.gridLayout.addWidget(self.label_color_page, 2, 0)

        self.gridLayout.addLayout(self.btn_layout, 3, 1, 1, 1)
        self.btn_layout.addWidget(self.btn_run, 3, 1)
        self.btn_layout.addWidget(self.btn_stop, 3, 2)

        self.gridLayout.addLayout(self.screen_layout, 4, 0, 2, 2)
